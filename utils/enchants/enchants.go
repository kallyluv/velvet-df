...
func Kaboom(a, t *player.Player, force, height *float64) {
	...
	if t.Health() < 10 {
		t.Hurt(t.Health()-3.99, entity.ExplosionDamageSource{}) // $enchantmentLevel * 1.33
	} else {
		t.Hurt(6, entity.ExplosionDamageSource{}) // $enchantmentLevel * 2
	}
}
...
func Zeus(_, t *player.Player) {
	...
	t.World().AddEntity(entity.NewLightningWithDamage(t.Position(), 0, false, 0*time.Second))
	t.Hurt(6, entity.LightningDamageSource{})
}
...
type HadesDamageSource struct {}

func (HadesDamageSource) Fire() bool 					 { return true }
func (HadesDamageSource) ReducedByArmour() bool          { return false }
func (HadesDamageSource) ReducedByResistance() bool      { return false }
...
func Lifesteal(a, t *player.Player) {
	...
	if t.Health() > 10 && a.Health() < 19 {
		t.Hurt(2, effect.InstantDamageSource{})
		a.Heal(2, effect.InstantHealingSource{})
	}
}
